...
namespace MagicaCloth2
{
    public class TeamManager : IManager, IValid
    {
        ...
        public const int Flag_KeepTeleport = 11; // 姿勢保持テレポート
        public const int Flag_InertiaShift = 12; // 慣性全体シフト
        ...
        internal void AlwaysTeamUpdate()
        {
            ...
            foreach (var cprocess in clothSet)
            {
                ...
                if (syncCloth && tdata.syncTeamId > 0)
                {
                    ...
                    // パラメータ同期
                    // 同期中は一部のパラメータを連動させる
                    var syncParam = syncCloth.SerializeData.GetClothParameters();
                    var clothParam = GetParameters(teamId);
                    clothParam.inertiaConstraint.worldInertia = syncParam.inertiaConstraint.worldInertia;
                    clothParam.inertiaConstraint.movementSpeedLimit = syncParam.inertiaConstraint.movementSpeedLimit;
                    clothParam.inertiaConstraint.rotationSpeedLimit = syncParam.inertiaConstraint.rotationSpeedLimit;
                    clothParam.inertiaConstraint.teleportMode = syncParam.inertiaConstraint.teleportMode;
                    clothParam.inertiaConstraint.teleportDistance = syncParam.inertiaConstraint.teleportDistance;
                    clothParam.inertiaConstraint.teleportRotation = syncParam.inertiaConstraint.teleportRotation;
                    SetParameters(teamId, clothParam);
        ...
        struct CalcCenterAndInertiaAndWindJob : IJobParallelFor
        {
            ...
            public void Execute(int teamId)
            {
                ...
                else
                {
                    ...
                    movementShift = tdata.IsKeepReset ? 1.0f : movementShift; // Keepテレポートはシフト量100%で実装
                    rotationShift = tdata.IsKeepReset ? 1.0f : rotationShift; // Keepテレポートはシフト量100%で実装
